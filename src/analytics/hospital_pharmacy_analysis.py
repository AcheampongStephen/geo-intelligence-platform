...
def calculate_coverage(
    ...
    """
    Count actual pharmacy locations within a
    specified radius of each hospital.

    Uses an inner spatial join so hospitals with
    no pharmacy matches are not incorrectly counted
    as having one pharmacy.
    """
    ...
    counts = joined.groupby("osm_id_hospital").size().rename("pharmacy_count")
...
def main():
    ...
    # --------------------------------
    # Add 1 km coverage counts.
    #
    # Hospitals with no matches receive
    ...
    # for mapping and interoperability.
    # --------------------------------
